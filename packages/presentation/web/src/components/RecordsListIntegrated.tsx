...
export const RecordsListIntegrated = forwardRef<RecordsListIntegratedRef, RecordsListIntegratedProps>(
  ({ records, onEdit, onDelete, searchQuery, onNavigateUp, onRecordUpdated, onRecordDeleted, ...props }, ref) => {
    const { updateRecordUseCase, deleteRecordUseCase } = useApplicationContext();
    const [isUpdating, setIsUpdating] = useState(false);
    const [isDeleting, setIsDeleting] = useState(false);
    const recordsListRef = useRef<RecordsListRef>(null);

    const updateRecord = useCallback(async (id: string, content: string): Promise<void> => {
    ...
    useImperativeHandle(ref, (): RecordsListIntegratedRef => ({
      focusFirst: (): void => {
    ...
    const loadingMessage = isUpdating ? 'Updating record...' : isDeleting ? 'Deleting record...' : '';
