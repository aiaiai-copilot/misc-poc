import { useState, useRef, KeyboardEvent, forwardRef, useImperativeHandle, useMemo, useCallback, memo } from 'react';
...
const highlightTags = (tags: string[], searchTerms: string[], searchQuery: string): string | JSX.Element[] => {
...
const RecordItem = memo<RecordItemProps>(({ index, style, data }) => {
  const { records, editingId, searchTerms, searchQuery, onEdit, onDelete, onKeyDown, itemRefs } = data;
...
const RecordItemOptimized = memo<RecordItemOptimizedProps>(({
  ...
  const handleKeyDownLocal = useCallback((e: KeyboardEvent<HTMLDivElement>) => {
    onKeyDown(e, index);
  }, [onKeyDown, index]);
...
const RecordsListComponent = memo(forwardRef<RecordsListRef, RecordsListProps>(({ records, onEdit, onDelete, searchQuery = '', onNavigateUp }, ref) => {
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent<HTMLDivElement>, index: number): void => {
    switch (e.key) {
      case 'ArrowUp':
        e.preventDefault();
        if (index > 0) {
  ...
  const searchTerms = useMemo(() => searchQuery.trim().split(/\s+/).filter(Boolean), [searchQuery]);
    ...
    <div className="w-full max-w-6xl mx-auto space-y-2" data-testid="records-list">
      {visibleRecords.map((record, index) => (
        <RecordItemOptimized
          ...
          itemRef={(el): void => { itemRefs.current[index] = el; }}
        />
      ))}
    </div>
  );
